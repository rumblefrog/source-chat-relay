...
If all goes well and the game server successfully connects to the relay server, you should see an entity from each game server when you run `r/entities` as a command to the Discord bot (If you don't, see [Troubleshooting](https://rumblefrog.me/source-chat-relay/troubleshooting)
...
> Note that you cannot use these commands within private message, it must be used within a server
> Note that you need to have `Link Preview` enabled within your Discord to view the command outputs correctly
